-- =====================================================
-- TITAN PPL: JEFF NIPPARD S+ PROTOCOL - VERSIÓN FINAL
-- Con TODAS las rutinas completas que especificaste
-- =====================================================
...
-- LEGS A (Máquinas/Estable - VERSIÓN ACTUALIZADA)
('Smith Machine Squat', 'Legs', 'Machine', 'compound', 'legs', 'intermediate', ARRAY['Quads', 'Glutes'], ARRAY['Core'], 5, 8, 300, '[LEGS A] S Tier. Tempo 3-1-0. RIR 1. Cue: Pies ligeramente adelantados. Baja profundo hasta que los gemelos toquen isquios. Usa los seguros.'),
('Weighted 45° Back Extension', 'Legs', 'Bodyweight', 'compound', 'legs', 'intermediate', ARRAY['Hamstrings', 'Glutes'], ARRAY['Back'], 8, 12, 180, '[LEGS A] S Tier (Reemplazo RDL). Tempo 2-1-2. Cue: Abraza un disco. Encorva un poco la espalda alta, pero empuja con la cadera contra el cojín. Siente el estiramiento en femorales.'),
('Standing Calf Raise', 'Legs', 'Machine', 'isolation', 'legs', 'beginner', ARRAY['Calves'], ARRAY[]::TEXT[], 12, 20, 60, '[LEGS A] A Tier. Pausa 2s abajo. Explosivo arriba. Myo-reps al final.'),
...
-- LEGS A (3 ejercicios - VERSIÓN MÁQUINAS/ESTABLE)
DO $$
DECLARE
  legs_a_id UUID;
BEGIN
  INSERT INTO health_routines (name, description, routine_type, difficulty, estimated_duration_minutes, is_template, is_active)
  VALUES ('Legs A - Máquinas & Seguridad', 'Día de piernas con máquinas para llegar al fallo sin riesgo.', 'legs', 'intermediate', 70, true, false)
  RETURNING id INTO legs_a_id;

  INSERT INTO health_routine_exercises (routine_id, exercise_id, exercise_order, target_sets, target_reps_min, target_reps_max, rest_seconds, rpe_target, notes)
  SELECT legs_a_id, id, 1, 4, 5, 8, 300, 9.0, 'Usa los seguros. Profundidad máxima.' FROM health_exercises WHERE name = 'Smith Machine Squat'
  UNION ALL
  SELECT legs_a_id, id, 2, 4, 8, 12, 180, 9.0, 'Abraza disco. Empuja cadera contra cojín.' FROM health_exercises WHERE name = 'Weighted 45° Back Extension'
  UNION ALL
...
BEGIN
  INSERT INTO health_routines (name, description, routine_type, difficulty, estimated_duration_minutes, is_template, is_active)
  VALUES ('Legs B - Máquinas & Unilateral', 'Día de piernas enfocado en máquinas.', 'legs', 'intermediate', 70, true, false)
